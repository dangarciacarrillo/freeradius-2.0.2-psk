				       ...
				       struct eap_psk_data *data, u8 id)
{
	...
	wpa_printf(MSG_INFO, "EAP-PSK: PSK-1 (sending)");
				       ...
				       struct eap_psk_data *data, u8 id)
{
	...
	wpa_printf(MSG_INFO, "EAP-PSK: PSK-3 (sending)");
	...
	wpa_hexdump_key(MSG_INFO, "EAP-PSK: MSK", data->msk, EAP_MSK_LEN);
	wpa_hexdump_key(MSG_INFO, "EAP-PSK: EMSK", data->emsk, EAP_EMSK_LEN);
...
static struct wpabuf * eap_psk_buildReq(struct eap_sm *sm, void *priv, u8 id)
{
	...
	default:
		wpa_printf(MSG_INFO, "EAP-PSK: Unknown state %d in buildReq",
			   data->state);
		break;
	}
	return NULL;
			     ...
			     struct wpabuf *respData)
{
	...
	wpa_printf(MSG_INFO, "EAP-PSK: received frame: T=%d", t);

	if (data->state == PSK_1 && t != 1) {
		wpa_printf(MSG_INFO, "EAP-PSK: Expected PSK-2 - "
			   "ignore T=%d", t);
		return TRUE;
	}

	if (data->state == PSK_3 && t != 3) {
		wpa_printf(MSG_INFO, "EAP-PSK: Expected PSK-4 - "
			   "ignore T=%d", t);
		return TRUE;
	}
	...
	if ((t == 1 && len < sizeof(struct eap_psk_hdr_2)) ||
		...
		wpa_printf(MSG_INFO, "EAP-PSK: Too short frame");
		return TRUE;
	}
			      ...
			      struct wpabuf *respData)
{
	...
	wpa_printf(MSG_INFO, "EAP-PSK: Received PSK-2");
	...
	printf_hex(data->id_p, data->id_p_len);

	if (eap_user_get(sm, data->id_p, data->id_p_len, 0) < 0) {
		wpa_hexdump_ascii(MSG_INFO, "EAP-PSK: unknown ID_P",
				  data->id_p, data->id_p_len);
		data->state = FAILURE;
		return;
	}
	...
	if (sm->user->password == NULL ||
		...
		wpa_hexdump_ascii(MSG_INFO, "EAP-PSK: invalid password in "
	...
	wpa_hexdump_key(MSG_INFO, "EAP-PSK: USER PASSWORD", sm->user->password, 16);
	printf_hex(sm->user->password,16);
	if (eap_psk_key_setup(sm->user->password, data->ak, data->kdk)) {
		data->state = FAILURE;
		return;
	}
	wpa_hexdump_key(MSG_INFO, "EAP-PSK: AK", data->ak, EAP_PSK_AK_LEN);
	printf_hex( data->ak, EAP_PSK_AK_LEN);	
	wpa_hexdump_key(MSG_INFO, "EAP-PSK: KDK", data->kdk, EAP_PSK_KDK_LEN);
	printf_hex(data->kdk, EAP_PSK_KDK_LEN);
	...
	pos += data->id_s_len;
	wpa_hexdump_key(MSG_INFO, "EAP-PSK: ID_S", data->id_s, data->id_s_len);
	printf_hex(data->id_s, data->id_s_len);

	os_memcpy(pos, data->rand_s, EAP_PSK_RAND_LEN);
	pos += EAP_PSK_RAND_LEN;
	wpa_hexdump_key(MSG_INFO, "EAP-PSK: RAND_S", data->rand_s, EAP_PSK_RAND_LEN);
	printf_hex(data->rand_s, EAP_PSK_RAND_LEN);

	os_memcpy(pos, data->rand_p, EAP_PSK_RAND_LEN);
	wpa_hexdump_key(MSG_INFO, "EAP-PSK: RAND_P", data->rand_p, EAP_PSK_RAND_LEN);
	printf_hex(data->rand_p, EAP_PSK_RAND_LEN);

	wpa_hexdump_key(MSG_INFO, "EAP-PSK: CMAC-AES-128(AK, ID_P||ID_S||RAND_S||RAND_P)\n", buf, buflen);
	printf_hex(buf, buflen);
	...
	os_free(buf);
	wpa_hexdump(MSG_INFO, "EAP-PSK: RECEIVED MAC_P", resp->mac_p, EAP_PSK_MAC_LEN);
	wpa_hexdump(MSG_INFO, "EAP-PSK: CALCULATED MAC_P", mac, EAP_PSK_MAC_LEN);
			      ...
			      struct wpabuf *respData)
{
	...
	wpa_printf(MSG_INFO, "EAP-PSK: Received PSK-4");
	...
	if (pos[0] == 0 && pos[1] == 0 && pos[2] == 0 && pos[3] == 0) {
		wpa_printf(MSG_INFO, "EAP-PSK: Nonce did not increase");
		return;
	}
	...
	wpa_hexdump(MSG_INFO, "EAP-PSK: Decrypted PCHANNEL message",
		    decrypted, left);

	/* Verify R flag */
	switch (decrypted[0] >> 6) {
	case EAP_PSK_R_FLAG_CONT:
		wpa_printf(MSG_INFO, "EAP-PSK: R flag - CONT - unsupported");
		data->state = FAILURE;
		break;
	case EAP_PSK_R_FLAG_DONE_SUCCESS:
		wpa_printf(MSG_INFO, "EAP-PSK: R flag - DONE_SUCCESS");
		data->state = SUCCESS;
		break;
	case EAP_PSK_R_FLAG_DONE_FAILURE:
		wpa_printf(MSG_INFO, "EAP-PSK: R flag - DONE_FAILURE");
		data->state = FAILURE;
		break;
	}
	os_free(decrypted);
